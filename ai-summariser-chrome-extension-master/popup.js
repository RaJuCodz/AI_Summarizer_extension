  ...
  chrome.storage.sync.get(["geminiApiKey"], async (result) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
      chrome.tabs.sendMessage(
        tab.id,
        { type: "GET_ARTICLE_TEXT_AND_TITLE" },
        async (res) => {
          if (!res || !res.text) {
            resultDiv.innerText =
          ...
          try {
            ...
            saveSummaryToHistory(
              ...
              summaryType,
              customPrompt,
              res.title || tab.title || "Untitled Article"
            );
            renderHistory();
          } catch (error) {
...
async function getGeminiSummary(text, summaryType, apiKey, customPrompt = "") {
...
function renderHistory() {
  ...
  chrome.storage.local.get(["summaryHistory"], (result) => {
    ...
    historyList.innerHTML =
      history.length === 0
        ? '<li style="color:#888;">No history yet.</li>'
        : history
            .map(
              (item, idx) =>
                `<li style="margin-bottom:10px;"><strong>${
                  item.articleTitle ? item.articleTitle : "Untitled Article"
                }</strong> <span style="color:#888;font-size:0.9em;">${new Date(
                  item.date
                ).toLocaleString()}</span><br><span style="color:#222;">${item.summary.replace(
                  /</g,
                  "&lt;"
                )}</span></li>`
            )
